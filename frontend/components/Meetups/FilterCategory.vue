      ...
      <ComboboxOptions static class="my-3 block w-full divide-y divide-zinc-100 overflow-auto rounded-md">
        <ComboboxOption v-for="item in filteredElements" v-slot="{ active, selected }" :key="item.name" :value="item.name" as="div">
          <li :class="['flex justify-between cursor-default select-none', active ? 'bg-zinc-100 text-zinc-700' : 'text-zinc-700', selected ? 'bg-zinc-200' : '']">
            <div class="ml-3 mr-1 flex items-center truncate py-3">
            ...
            <span v-if="selected" :class="['flex items-center hover:text-red-200 mr-1 sm:mr-4', active ? 'text-zinc-700' : 'text-zinc-500']">
