...
<template>
  ...
  <div v-if="!error" class="mx-auto my-12">
    <div class="border-b border-zinc-200 inline-block w-full items-center justify-between bg-white text-zinc-700 sm:flex">
      <form class="block w-full justify-center sm:order-2 sm:flex sm:w-3/5">
        <div class="w-full sm:w-full md:m-4">
          <div class="relative mt-1 h-12">
            ...
            <input v-model="searchMeetup" type="text" name="meetup" class="border-r bg-zinc-100 block h-full w-full border-zinc-300 pr-4 pl-12 shadow-sm placeholder:text-zinc-400 focus:border-zinc-500 focus:ring-zinc-500 sm:text-sm" placeholder="#23" @keydown.enter.prevent>
          </div>
        </div>
      </form>
      <div class="inline-block w-1/2 h-fit  border-r border-zinc-200 m-0 p-0 justify-center sm:order-1 sm:flex sm:w-1/5">
      ...
      <div class="inline-block w-1/2 h-full justify-center sm:order-3 sm:flex sm:w-1/5">
